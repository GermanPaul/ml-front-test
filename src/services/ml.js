...
module.exports = {
  ...
  getItem: (req, res) => {
    ...
    axios
      .all([axios.get(itemUrl), axios.get(`${itemUrl}/description`)])
      .then(
        axios.spread((...responses) => {
          const itemResponse = responses[0].data;
          const descriptionResponse = responses[1].data;
          formattedResponse = Item.formatResponse(
            itemResponse,
            descriptionResponse
          );
          const categoryUrl = categoryEndpoint.replace(
            '{id}',
            responses[0].data.category_id
          );
